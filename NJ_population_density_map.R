...
pacman::p_load("tidyverse","tidycensus","sf","ggsn","cowplot","viridis")
###########################################################################################
...
### Read in NJ counties shapefile ###
NJ_counties<-st_read(dsn= getwd(),layer="NJ_counties")
###########################################################################################
...
NJ_pop_estimates<-get_estimates(geography = "county",state = "NJ",
                                variables = "DENSITY",geometry = T)
###########################################################################################
### Make breaks for legend ###
NJ_pop_estimates$brks <- cut(NJ_pop_estimates$value, 
...
pop_map<-ggplot()+
  geom_sf(data = NJ_pop_estimates,aes(fill=brks))+
  ...
  xlab("")+
  ylab("")+
  labs(subtitle = "Source: U.S. Census Bureau Population Estimates API")+
  ...
  scale_color_manual("Population per County (2017 Estimates)",
